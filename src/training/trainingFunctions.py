...
def accuracy_per_class(output, target):
    ...
    for i, cur_target in enumerate(target):
        # if cur_target == pred[i]:
        #     # print(cur_target, pred[i])
        if cur_target == 1:
            total_racist += 1
    ...
    if total_racist == 0 : 
        print('hmmmm total racist is 0?')
        correct_racist = 1
        total_racist = 1
    if total_notRacist == 0 : 
        print('hmmmm total not racist is 0?')
        correct_notRacist = 1
        total_notRacist = 1
    ...
    # print(f"racist: {correct_racist} / {total_racist} = {acc_racist}")
    # print(f"not racist: {correct_notRacist} / {total_notRacist} = {acc_notRacist}")
