...
class CustomDatasetFCM(Dataset):
    ...
    def set_tweet_ids(self, split_filepath):
        with open(split_filepath, "r", encoding="utf-8") as file:
            ...
            # print('tweet ids length:', len(self.tweet_ids))
            # for i in range(len(self.tweet_ids)):
            #     print(f'{i}:',self.tweet_ids[i])
            # print('-1:',self.tweet_ids[-1])
        ...
        try:
            for i,line in enumerate(open(filepath)):
                ...
                if tweet_id in self.tweet_ids:
                    ...
                    self.tweets[index, :] = arr
                    
                    # print(f"{tweet_id}:{self.tweets[index][0]}")
        except Exception as e:
            print('error 101', e)
        ...
        try:
            for i,line in enumerate(open(filepath)):
                ...
                if tweet_id in self.tweet_ids:
                    ...
                    # print(f"{tweet_id}:{self.img_texts[index][0]}")
        except Exception as e:
            print('error 102', e)
